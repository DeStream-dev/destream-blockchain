...
namespace DeStream.Bitcoin.Features.LightWallet.Tests
{
    ...
    public class LightWalletFixedFeePolicyTest
    {
        ...
        [Fact]
        public void MinTxFee_OnDeStreamNetwork_IsDeStreamDefault()
        {
            ILoggerFactory loggerFactory = new LoggerFactory();
            NodeSettings nodeSettings = new NodeSettings(Network.DeStreamTest);
            LightWalletFixedFeePolicy policy = new LightWalletFixedFeePolicy(loggerFactory, nodeSettings);
            Assert.Equal(new FeeRate(Network.DeStreamTest.FallbackFee), policy.FallbackTxFeeRate);
        }
