...
namespace DeStream.Bitcoin.Api.Tests
{
    /// <summary>
    ...
    public class ApiSettingsTest : TestBase
    {
        ...
        /// <summary>
        /// Tests that if no API settings are passed and we're on the DeStream network, the defaults settings are used.
        /// </summary>
        [Fact]
        public void GivenNoApiSettingsAreProvided_AndOnDeStreamNetwork_ThenDefaultSettingAreUsed()
        {
            // Arrange.
            ...
            // Assert.
            Assert.Equal(ApiSettings.DefaultDeStreamApiPort, settings.ApiPort);
            Assert.Equal(new Uri($"{ApiSettings.DefaultApiHost}:{ApiSettings.DefaultDeStreamApiPort}"), settings.ApiUri);
        }
        ...
        /// <summary>
        /// Tests that if a custom API URI is passed and we're on the DeStream network, the bitcoin port is used in conjunction with the passed API URI.
        /// </summary>
        [Fact]
        public void GivenApiUriIsProvided_AndGivenDeStreamNetwork_ThenApiUriIsUsedWithDefaultDeStreamApiPort()
        {
            // Arrange.
            Transaction.TimeStamp = true;
            Block.BlockSignature = true;
            ...
            // Assert.
            Assert.Equal(ApiSettings.DefaultDeStreamApiPort, settings.ApiPort);
            Assert.Equal(new Uri($"{customApiUri}:{ApiSettings.DefaultDeStreamApiPort}"), settings.ApiUri);
        }
        ...
        /// <summary>
        /// Tests that if we're on the DeStream main network, the port used in the API is the right one.
        /// </summary>
        [Fact]
        public void GivenDeStreamMainnet_ThenUseTheCorrectPort()
        {
            // Arrange.
            Transaction.TimeStamp = true;
            Block.BlockSignature = true;
            NodeSettings nodeSettings = NodeSettings.Default(Network.DeStreamMain);
        ...
        /// <summary>
        /// Tests that if we're on the DeStream test network, the port used in the API is the right one.
        /// </summary>
        [Fact]
        public void GivenDeStreamTestnet_ThenUseTheCorrectPort()
        {
            // Arrange.
            Transaction.TimeStamp = true;
            Block.BlockSignature = true;
            NodeSettings nodeSettings = NodeSettings.Default(Network.DeStreamTest);
            ...
            // Assert.
            Assert.Equal(ApiSettings.TestDeStreamApiPort, settings.ApiPort);
        }
    }
}
