...
using DeStream.Bitcoin.Utilities.JsonConverters;
using Xunit;
...
namespace DeStream.Bitcoin.Features.Wallet.Tests
{
    public class WalletManagerTest : LogsTestBase, IDisposable, IClassFixture<WalletFixture>
    {
        ...
        public void CreateWalletWithoutPassphraseOrMnemonicCreatesWalletUsingPassword()
        {
            ...
            var chain = new ConcurrentChain(Network.DeStreamMain);
            ...
            var walletManager = new WalletManager(this.LoggerFactory.Object, Network.DeStreamMain, chain, NodeSettings.Default(), new Mock<WalletSettings>().Object,
            ...
            Assert.Equal(Network.DeStreamMain, expectedWallet.Network);
            ...
            {
                Assert.Equal(CoinType.DeStream, expectedWallet.AccountsRoot.ElementAt(i).CoinType);
            ...
            Assert.Equal(actualWallet.EncryptedSeed, mnemonic.DeriveExtKey(password).PrivateKey.GetEncryptedBitcoinSecret(password, Network.DeStreamMain).ToWif());
            Assert.Equal(expectedWallet.EncryptedSeed, mnemonic.DeriveExtKey(password).PrivateKey.GetEncryptedBitcoinSecret(password, Network.DeStreamMain).ToWif());
        }
        ...
        public void CreateWalletWithPasswordAndPassphraseCreatesWalletUsingPasswordAndPassphrase()
        {
            ...
            Assert.Equal(actualWallet.EncryptedSeed, mnemonic.DeriveExtKey(passphrase).PrivateKey.GetEncryptedBitcoinSecret(password, Network.DeStreamMain).ToWif());
            Assert.Equal(expectedWallet.EncryptedSeed, mnemonic.DeriveExtKey(passphrase).PrivateKey.GetEncryptedBitcoinSecret(password, Network.DeStreamMain).ToWif());
        }
        ...
        public void LoadWalletWithExistingWalletLoadsWalletOntoManager()
        {
            ...
            var walletManager = new WalletManager(this.LoggerFactory.Object, Network.DeStreamMain, new Mock<ConcurrentChain>().Object, NodeSettings.Default(), new Mock<WalletSettings>().Object,
                                                dataFolder, new Mock<IWalletFeePolicy>().Object, new Mock<IAsyncLoopFactory>().Object, new NodeLifetime(), DateTimeProvider.Default);
        ...
        public void LastBlockHeightWithWalletsReturnsLowestLastBlockSyncedHeightForAccountRootsOfManagerCoinType()
        {
            var walletManager = new WalletManager(this.LoggerFactory.Object, Network.Main, new Mock<ConcurrentChain>().Object, NodeSettings.Default(), new Mock<WalletSettings>().Object,
                CreateDataFolder(this), new Mock<IWalletFeePolicy>().Object, new Mock<IAsyncLoopFactory>().Object, new NodeLifetime(), DateTimeProvider.Default);
            var wallet = this.walletFixture.GenerateBlankWallet("myWallet", "password");
            wallet.AccountsRoot.ElementAt(0).CoinType = CoinType.DeStream;
        ...
        public void GetSpendableTransactionsReturnsTransactionsGivenBlockHeight()
        {
            ...
            wallet.AccountsRoot.Add(new AccountRoot()
            {
                ...
                {
                    new HdAccount {
                        ExternalAddresses = WalletTestsHelpers.CreateUnspentTransactionsOfBlockHeights(Network.DeStreamMain, 8,9,10),
                        InternalAddresses = WalletTestsHelpers.CreateUnspentTransactionsOfBlockHeights(Network.DeStreamMain, 8,9,10)
                    }
                }
            });

            var wallet2 = this.walletFixture.GenerateBlankWallet("myWallet2", "password");
            wallet2.AccountsRoot.ElementAt(0).CoinType = CoinType.DeStream;
            wallet2.AccountsRoot.ElementAt(0).Accounts.Add(new HdAccount
            {
                ExternalAddresses = WalletTestsHelpers.CreateUnspentTransactionsOfBlockHeights(Network.DeStreamMain, 1, 3, 5, 7, 9, 10),
                InternalAddresses = WalletTestsHelpers.CreateUnspentTransactionsOfBlockHeights(Network.DeStreamMain, 2, 4, 6, 8, 9, 10)
            });
        ...
        {
            var result = HdOperations.CreateHdPath((int)CoinType.DeStream, 4, 3, true);
        ...
        {
            var result = HdOperations.CreateHdPath((int)CoinType.DeStream, 4, 3, false);
