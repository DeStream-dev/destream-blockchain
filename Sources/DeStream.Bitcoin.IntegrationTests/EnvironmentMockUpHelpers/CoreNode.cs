...
using NBitcoin.RPC;
using DeStream.Bitcoin.Configuration.Logging;
...
using static DeStream.Bitcoin.BlockPulling.BlockPuller;
...
namespace DeStream.Bitcoin.IntegrationTests.EnvironmentMockUpHelpers
{
    public class CoreNode
    {
        ...
        public FullNode FullNode
        {
            get
            {
                if (this.runner is DeStreamBitcoinPosRunner)
                    return ((DeStreamBitcoinPosRunner)this.runner).FullNode;

                return ((DeStreamBitcoinPowRunner)this.runner).FullNode;
            }
        }
        ...
        public bool AddToDeStreamMempool(Transaction trx)
        {
            var fullNode = (this.runner as DeStreamBitcoinPowRunner).FullNode;
            var state = new MempoolValidationState(true);

            return fullNode.MempoolManager().Validator.AcceptToMemoryPool(state, trx).Result;
        }

        public List<uint256> GenerateDeStreamWithMiner(int blockCount)
        {
        ...
        [Obsolete("Please use GenerateDeStreamWithMiner instead.")]
        public Block[] GenerateDeStream(int blockCount, List<Transaction> passedTransactions = null, bool broadcast = true)
        {
            var fullNode = (this.runner as DeStreamBitcoinPowRunner).FullNode;
