...
using DeStream.Bitcoin.Features.Consensus.CoinViews;
...
namespace DeStream.Bitcoin.IntegrationTests.EnvironmentMockUpHelpers
{
    ...
    public class NodeBuilder : IDisposable
    {
            ...
            for (int retry = 0; retry < 2; retry++)
            {
                ...
                if (tryKill)
                {
                    ...
                    foreach (var bitcoind in Process.GetProcessesByName("bitcoind"))
                        if (bitcoind.MainModule.FileName.Contains("DeStream.Bitcoin.IntegrationTests"))
                            bitcoind.Kill();
        ...
        public CoreNode CreateDeStreamPowNode(bool start = false, Action<IFullNodeBuilder> callback = null)
        {
            string child = this.CreateNewEmptyFolder();
            var node = new CoreNode(child, new DeStreamBitcoinPowRunner(callback), this, Network.RegTest);
            this.Nodes.Add(node);
            if (start)
                node.Start();
            return node;
        ...
        public CoreNode CloneDeStreamNode(CoreNode cloneNode)
        {
            var node = new CoreNode(cloneNode.Folder, new DeStreamBitcoinPowRunner(), this, Network.RegTest, false);
            this.Nodes.Add(node);
